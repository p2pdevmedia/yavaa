...
test('signs up through the UI and provisions selectable roles', async ({ page }) => {
  ...
  try {
    ...
    const profileSelectionHeading = page.getByRole('heading', { name: /Elegí tu perfil/i });
    const authError = page.getByTestId('auth-error');

    const outcome = await Promise.race([
      confirmationMessage.waitFor({ state: 'visible' }).then(() => 'confirmation' as const),
      profileSelectionHeading.waitFor({ state: 'visible' }).then(() => 'profile-selection' as const),
      authError.waitFor({ state: 'visible' }).then(() => 'auth-error' as const),
      page.waitForTimeout(15_000).then(() => 'timeout' as const)
    ]);

    if (outcome === 'profile-selection') {
      await expect(profileSelectionHeading).toBeVisible();
