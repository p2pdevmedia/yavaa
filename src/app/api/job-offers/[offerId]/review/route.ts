...
import { reviewReadyJobOffer } from '@/lib/job-offers';
import { resolveRequestAuth } from '@/lib/request-auth';

type JobOfferReviewRouteContext = {
  params: Promise<{
    offerId: string;
  }>;
};
...
export async function POST(request: NextRequest, context: JobOfferReviewRouteContext): Promise<NextResponse> {
  ...
  const result = await reviewReadyJobOffer(auth, offerId, json.body);
