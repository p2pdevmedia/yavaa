...
import type { JobPaymentSummary } from '@/lib/job-offers';

type WorkerJobPaymentsProps = {
  offerId: string;
  initialPayments: JobPaymentSummary[];
...
export function WorkerJobPayments({
  offerId,
  initialPayments,
  currentUserId,
  canRegisterPayments
}: WorkerJobPaymentsProps) {
  const receiptInputRef = useRef<HTMLInputElement>(null);
  const [payments, setPayments] = useState<JobPaymentSummary[]>(initialPayments);
  ...
  const amountErrorId = `worker-job-payment-${offerId}-amount-error`;
  const paidAtErrorId = `worker-job-payment-${offerId}-paid-at-error`;
  const descriptionErrorId = `worker-job-payment-${offerId}-description-error`;
  const receiptErrorId = `worker-job-payment-${offerId}-receipt-error`;
  const formErrorId = `worker-job-payment-${offerId}-form-error`;
  const statusId = `worker-job-payment-${offerId}-status`;

  const orderedPayments = useMemo(
    () => [...payments].sort((left, right) => new Date(right.paidAt).getTime() - new Date(left.paidAt).getTime()),
    [payments]
  );

  function getAuthorLabel(createdById: string): string {
    return createdById === currentUserId ? 'Vos' : 'Cliente';
  }
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (isSubmitting || !canRegisterPayments) {
      return;
    }
  ...
  return (
    <section className="space-y-4 rounded-[28px] border border-border bg-card p-6 shadow-soft">
      <div className="space-y-1">
        <p className="text-xs font-extrabold uppercase tracking-[0.14em] text-primary">Pagos del trabajo</p>
        <h2 className="font-display text-2xl font-bold tracking-normal text-foreground">Pagos registrados</h2>
      </div>

      <div className="space-y-3 rounded-[18px] border border-border bg-background p-3">
        {orderedPayments.length === 0 ? (
          ...
          orderedPayments.map((payment) => (
            <article key={payment.id} className="space-y-1 rounded-[16px] bg-card px-3 py-2">
      ...
      {canRegisterPayments ? (
        ...
        >
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="space-y-2">
              <Label htmlFor={`worker-job-payment-${offerId}-amount`}>Monto</Label>
              <Input
                id={`worker-job-payment-${offerId}-amount`}
                name="amountPesos"
                type="number"
                inputMode="numeric"
                min="1"
            ...
            <div className="space-y-2">
              <Label htmlFor={`worker-job-payment-${offerId}-paid-at`}>Fecha de pago</Label>
              <Input
                id={`worker-job-payment-${offerId}-paid-at`}
                name="paidAt"
                type="datetime-local"
                value={paidAt}
          ...
          <div className="space-y-2">
            <Label htmlFor={`worker-job-payment-${offerId}-description`}>Descripción</Label>
            <textarea
              id={`worker-job-payment-${offerId}-description`}
              name="description"
          ...
          <div className="space-y-2">
            <Label htmlFor={`worker-job-payment-${offerId}-receipt`}>Comprobante opcional</Label>
            <Input
              ref={receiptInputRef}
              id={`worker-job-payment-${offerId}-receipt`}
              name="receipt"
              type="file"
        ...
        <p className="text-sm font-semibold text-muted-foreground">Los pagos quedan disponibles como historial.</p>
      )}
    </section>
  );
}
